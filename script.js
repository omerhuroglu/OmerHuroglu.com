(() => {
  ...
  const SELECTORS = {
    ...
    messageReducedMotion: "[data-message-reduced-motion]",
  ...
  const reducedMotionMessageIndex = getReducedMotionMessageIndex(messageItems);
  ...
  function getReducedMotionMessageIndex(messageItems) {
    const reducedMotionMessage = getRequiredElement(SELECTORS.messageReducedMotion);
    const reducedMotionMessageIndex = messageItems.indexOf(reducedMotionMessage);

    if (reducedMotionMessageIndex === -1) {
      throw new Error(`Reduced-motion message must also match: ${SELECTORS.messageItem}`);
    }

    return reducedMotionMessageIndex;
  }
  ...
  function showReducedMotionMessage() {
    clearMessageTimeout();
    resetMessageState(reducedMotionMessageIndex);
